...
- Удаление диалога интегрировано в список с отдельным подтверждением,
  fail-closed блокировкой для незавершённой работы и ручной проверкой
  неизвестного результата. Поведение подтверждено UI-тестами; production
  rollout и проверка на живом Harness остаются отдельным эксплуатационным gate.
