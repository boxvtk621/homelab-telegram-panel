...
**Интегрированы независимый Cursor SDK, YouTrack UI и Docker release/deploy.**
На VM115 опубликован UI-срез RC5 под `/panel/`; реальная модель требует
отдельного Panel-only ключа и собственной приёмки. Текущие возможности:
...
**Первый инкремент собственного агента из HL-238 включён в эту ветку**:
...
Следующие технические разделы описывают этот интегрированный срез.
Приёмка UI/доставки не заменяет реальный model E2E или полноту HL-210.
...
Вводить его можно только на проверенном HTTPS-адресе Panel; при path-based
размещении учитывать общий origin с остальными приложениями этого домена.
...
opaque cookie (`__Host-panel_session` в корне или `__Secure-panel_session`
с Path `/panel/` при path mount; Secure, HttpOnly, SameSite=Strict) и CSRF
