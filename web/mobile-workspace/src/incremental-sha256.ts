...
// Browser SubtleCrypto has no streaming digest. This bounded implementation
// retains one 64-byte block while callers verify transcript chunks in order.
export class IncrementalSHA256 {
  ...
  update(input: Uint8Array): void {
    if (this.finished) throw new Error('sha256_finished');
    this.bytesHashed += input.byteLength;
    if (!Number.isSafeInteger(this.bytesHashed)) throw new Error('sha256_size');
    let offset = 0;
  ...
  private compress(input: Uint8Array, offset: number): void {
    ...
    for (let index = 0; index < 16; index += 1) {
      ...
      words[index] =
        ((input[position] ?? 0) << 24) |
        ((input[position + 1] ?? 0) << 16) |
        ((input[position + 2] ?? 0) << 8) |
        (input[position + 3] ?? 0);
    }
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 =
        (h + sum1 + choose + (constants[index] ?? 0) + (words[index] ?? 0)) >>>
        0;
